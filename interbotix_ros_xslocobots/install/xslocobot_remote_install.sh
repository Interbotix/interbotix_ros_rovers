...
if [ $ubuntu_version == "16.04" ]; then
  ...
  echo -e "Interbotix Remote Locobot only works with 16.04, 18.04, or 20.04"
  exit 1
fi
...
if [ ! -d "$INTERBOTIX_WS/src" ]; then
  echo "Installing Simulation/Visualization ROS packages for the Interbotix Locobot..."
...
echo -e "NOTE: Remember to comment out the ${ORANGE}source $INTERBOTIX_WS/devel/setup.bash${NC} and ${ORANGE}export ROS_MASTER_URI=http://$HOSTNAME.local:11311${NC} lines from the ~/.bashrc file when done using the Locobot! Then close out of your terminal and open a new one."
